...
def compute_class_weights(dataset, num_classes: int = NUM_CLASSES) -> torch.Tensor:
    ...
    Scans all frames once, counts valid pixels per class, then computes:
        w(c) = total_valid_pixels / (num_classes * count(c))

    This is the sklearn balanced class weight formula.
    Classes with zero pixels get weight 0 (they won't appear in loss anyway).
    Weights are normalized so they average to 1.0 across present classes.
    """
    ...
    for i in range(len(dataset)):
        ...
        # Exclude undefined (label 0) — not a real semantic class
        valid_labels = valid_labels[valid_labels != 0]
        for c in range(num_classes):
            counts[c] += (valid_labels == c).sum()
        if (i + 1) % 100 == 0:
            print(f"  Scanned {i+1}/{len(dataset)} frames...", end="\r")

    print(f"  Scanned {len(dataset)}/{len(dataset)} frames.")
    ...
    # Normalize so mean weight across present classes = 1.0
    weights[present] /= weights[present].mean()

    # Force TYPE_UNDEFINED (class 0) weight to 0 — excluded from training loss
    weights[0] = 0.0

    print("\nClass weights:")
    for c in range(num_classes):
        print(f"  {CLASS_NAMES[c]:<30} count={counts[c]:>10,}  weight={weights[c]:.4f}")
...
class LiDARDiffusionModule(L.LightningModule):
    def __init__(self, hparams: dict, class_weights: torch.Tensor | None = None):
        super().__init__()
        ...
        # Register class weights as a buffer so they move to the right device automatically
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss     = self.ddpm.loss(eps_pred, eps, valid, labels=labels, class_weights=self.class_weights)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Validation loss unweighted — gives a clean comparable signal across runs
        ...
        pred      = x0_est.argmax(dim=1).cpu()

        v      = valid.bool().cpu()
        pred_v = pred[v]
        true_v = labels.cpu()[v]

        # Exclude undefined pixels (label 0) from validation metrics
        defined = (true_v != 0)
    ...
    def on_validation_epoch_end(self):
        ...
        # Skip class 0 (TYPE_UNDEFINED) from mIoU
        valid_ious = [iou for c, iou in enumerate(ious) if c != 0 and not np.isnan(iou)]
...
def main(args):
    ...
    if args.num_val_segs > 0:
        ...
        val_loader = DataLoader(test_ds, **val_loader_kwargs)
    else:
    ...
    print("\n── Building model ──")
    hparams = vars(args)
    module  = LiDARDiffusionModule(hparams, class_weights=class_weights)
