"""DP3-style point diffusion (skeleton)."""
...
from .point_backbones import EdgeConvBackbone
from .base import PointCloudSegmentationModel


class DP3PointDiffusionDenoiser(nn.Module):
    def __init__(
        self,
        *,
        encoder: nn.Module,
        ...
        self.out_proj = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Linear(self.backbone.output_dim, self.num_classes),
        )

    def forward(
        self, x_t: torch.Tensor, t: torch.Tensor, model_inputs: torch.Tensor
    ) -> torch.Tensor:
...
class DP3PointDiffusionSegmenter(nn.Module):
    def __init__(
        self,
        *,
        num_classes: int,
        ...
        self.use_balanced_class_weights = bool(use_balanced_class_weights)
        self.validation_prediction_mode = str(validation_prediction_mode)
        ...
        self.denoiser = DP3PointDiffusionDenoiser(
    ...
    def forward(self, x_t: torch.Tensor, t: torch.Tensor, model_inputs: torch.Tensor) -> torch.Tensor:
        return self.denoiser(x_t, t, model_inputs)


class DP3PointDiffusionModule(PointCloudSegmentationModel):
    def __init__(
        self,
        num_classes: int = 23,
        learning_rate: float = 2e-4,
        ...
        effective_geometry_only = True if geometry_only is None else bool(geometry_only)
        self.save_hyperparameters()
        input_dim = point_input_dim(geometry_only=effective_geometry_only)
        self.model = DP3PointDiffusionSegmenter(
            num_classes=int(num_classes),
            point_input_dim=int(input_dim),
            encoder_out_dim=int(encoder_out_dim),
            encoder_use_layernorm=bool(encoder_use_layernorm),
            encoder_final_norm=str(encoder_final_norm),
            hidden_dim=int(hidden_dim),
            depth=int(depth),
            knn_k=int(knn_k),
            time_dim=int(time_dim),
            ...
            use_balanced_class_weights=bool(use_balanced_class_weights),
            validation_prediction_mode=str(validation_prediction_mode),
            geometry_only=effective_geometry_only,
        )
        self.ddpm = self.model.ddpm

    @property
    def geometry_only_enabled(self) -> bool:
        return True if self.hparams.geometry_only is None else bool(self.hparams.geometry_only)
